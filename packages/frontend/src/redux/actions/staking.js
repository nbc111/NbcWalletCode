...
import { selectAccountId, selectAccountSlice } from '../slices/account';
...
export const { staking } = createActions({
    STAKING: {
        ...
        UPDATE_ACCOUNT: async (balance, validators, accountId, validatorDepositMap) => {
            ...
            await Promise.all(
                validators.map(async (validator, i) => {
                    try {
                        const total = new BN(
                        ...
                        const deposit = new BN(
                            validatorDepositMap[validator.accountId] || '0'
                        );
                        validator.staked =
                            await validator.contract.get_account_staked_balance({
                                account_id: accountId,
                            });
                        ...
                        if (
                            !deposit.gt(ZERO) ||
                            new BN(validator.unclaimed).lt(MIN_DISPLAY_YOCTO)
                        ) {
                        ...
                        if (validator.unstaked.gt(MIN_DISPLAY_YOCTO)) {
                            const isAvailable =
                                await validator.contract.is_account_unstaked_balance_available(
                                    { account_id: accountId }
                                );
                            if (isAvailable) {
            ...
            return {
                ...
                totalUnclaimed: (totalUnclaimed.lt(MIN_DISPLAY_YOCTO)
                    ? ZERO
                    : totalUnclaimed
                ).toString(),
                totalPending: totalPending.toString(),
                totalAvailable: (totalAvailable.lt(MIN_DISPLAY_YOCTO)
                    ? ZERO
                    : totalAvailable
                ).toString(),
            };
        ...
        UPDATE_LOCKUP: async (
            ...
            let validator = validators.find(
                (validator) => validator.accountId === selectedValidator
            );
            ...
            try {
                total = new BN(
                    await validator.contract.get_account_total_balance({ account_id })
                );
                if (total.gt(ZERO)) {
                    validator.staked =
                        await validator.contract.get_account_staked_balance({
            ...
            async ({ accountId }) => {
                ...
                try {
                    ...
                    contract = await new Contract(
                        await wallet.getAccount(accountId),
                        lockupId,
                        { ...lockupMethods }
                    );
                } catch (e) {
    ...
    try {
        await dispatch(
            staking.getLockup({
                ...
                isOwner:
                    !externalAccountId ||
                    externalAccountId === selectAccountId(getState()),
            })
        );
    } catch (e) {
...
export const handleStakingUpdateAccount =
    (recentlyStakedValidators = [], exAccountId) =>
    async (dispatch, getState) => {
        ...
        const validators = selectStakingAllValidators(getState())
            .filter((validator) =>
                Object.keys(validatorDepositMap)
                    .concat(recentlyStakedValidators)
                    .includes(validator.accountId)
            )
            .map((validator) => ({ ...validator }));

        await dispatch(
            staking.updateAccount(balance, validators, accountId, validatorDepositMap)
        );
    };
...
export const handleStakingUpdateLockup = (exAccountId) => async (dispatch, getState) => {
    ...
    await dispatch(
        staking.updateLockup(contract, lockupId, exAccountId, accountId, validators)
    );
};

export const handleStakingAction =
    (action, validatorId, amount) => async (dispatch, getState) => {
        ...
        if (isLockup) {
            ...
            const lockupId = selectStakingLockupId(getState());
            await dispatch(
                staking[action].lockup(lockupId, amount, contract, validatorId)
            );
        } else {
            const contract = selectStakingFindContractByValidatorId(getState(), {
                validatorId,
            });
            await dispatch(
                staking[action].account(validatorId, amount, accountId, contract)
            );
        }
...
export const updateStaking =
    (currentAccountId, recentlyStakedValidators) => async (dispatch, getState) => {
        ...
        let currentAccount = selectStakingCurrentAccountbyAccountId(getState(), {
            accountId: currentAccountId,
        });

        if (!currentAccount) {
            currentAccount = selectStakingCurrentAccountbyAccountId(getState(), {
                accountId,
            });
...
export const getValidatorFarmData = createAsyncThunk(
    'staking/getValidatorFarmData',
    async ({ validator, accountId }, { dispatch }) => {
...
export const claimFarmRewards = (validatorId, token_id) => async (dispatch, getState) => {
    try {
        ...
        return validator.contract.claim(
            args,
            CONFIG.FARMING_CLAIM_GAS,
            CONFIG.FARMING_CLAIM_YOCTO
        );
    } catch (error) {
        throw error;
    }
